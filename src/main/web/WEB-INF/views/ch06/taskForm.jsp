    ...
    <script type="text/javascript">
        ...
        function readComments() {
            ...
            $.getJSON('/ch09/comment/list/${hasFormKey ? task.processInstanceId : taskFormData.task.processInstanceId}', function (datas) {
                $.each(datas.commentList, function (i, v) {
                    $('<li/>', {
                        html: function () {
                            var content = v.fullMessage;
                            content += "<span style='margin-left:1em;'></span>" + v.userId + "（" + datas.taskNameMap[v.taskId] + "）";
...
<body>
<h3>
    任务办理—[${hasFormKey ? task.name : taskFormData.task.name}]，流程定义ID：[${hasFormKey ? task.processDefinitionId : taskFormData.task.processDefinitionId}]</h3>
<hr/>
