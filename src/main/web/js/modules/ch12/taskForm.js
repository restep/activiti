...
function taskProperties() {
    ...
    $('.due-date-input').blur(function () {
        ...
        var value = $(this).val();
        if (value) {
            $.post('/ch06/task/changeProperty/' + taskId, {
...
function participants() {
    ...
    $('#sendAddParticipantsRequest').click(function () {
        ...
        $.post('/ch06/task/participant/add/' + taskId, {
            userId: userIds,
            type: types
        }, function (resp) {
            if (resp == 'success') {
    ...
    $('.link-delete').click(function () {
        var ele = this;
        $.post('/ch06/task/participant/delete/' + taskId, {
...
function candidate() {
    ...
    $('#sendAddCandidateRequest').click(function () {
        ...
        $.post('/ch06/task/candidate/add/' + taskId, {
...
function subTask() {
    $('.subtask-delete').click(function () {
        ...
        location.href = '/ch06/task/delete/' + $(this).data('taskid');
    });
}
...
function attachment() {
    ...
    $('.attachment-delete').click(function () {
        var ele = this;
        if (confirm('确定删除附件？')) {
            $.get('/ch12/attachment/delete/' + $(this).data('id'), function (resp) {
                if (resp == 'true') {
                    $(ele).parent().remove();
                } else {
                    alert('删除失败！');
...
function delegate() {
    ...
    $('#delegateUserSelect').change(function () {
        ...
        var value = $(this).val();
        if (value) {
            $.post('/ch06/task/delegate/' + taskId, {
